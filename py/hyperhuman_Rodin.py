    ...
    for file_info in files_info["list"]:
        ...
        with requests.get(file_info["url"], stream=True) as r:
            if file_path.endswith('.glb'):
                ...
                shaded = handle_image(kwargs['geometry']['model']['visual'].material.baseColorTexture)
                diffuse = handle_image(kwargs['geometry']['model']['visual'].material.baseColorTexture)
                normal = handle_image(kwargs['geometry']['model']['visual'].material.normalTexture)
                pbr = handle_image(kwargs['geometry']['model']['visual'].material.metallicRoughnessTexture)
...
class RodinMultipleImage3D(RodinImage3D):
    ...
    def main_func(self, api_key, seed_, geometry_file_format, material, quality, use_hyper, tier, condition_mode, image_1=None, image_2=None, image_3=None, image_4=None, image_5=None, prompt=None):
        ...
        model_path = self.process_request(api_key, images, prompt, condition_mode, seed_, geometry_file_format, material, quality, use_hyper, tier)
